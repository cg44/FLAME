...
readModel (input_file * inputfile, char *directory, model_data * modeldata)
{
  ...
  int states, state, /* attribute, transition, */ func, dest, functions,
    function;
  int note, messages, message, code, cdata, environment, define, value,
    codefile;
  ...
  while (reading == 1 && c != (char) EOF)
      ...
      else if (c == '>')
	{
	  ...
	  if (strcmp (current_string->array, "date") == 0)
	    {
	      date = 1;
	    }
	  if (strcmp (current_string->array, "/date") == 0)
	  ...
	  if (strcmp (current_string->array, "author") == 0)
	    {
	      author = 1;
	    }
	  if (strcmp (current_string->array, "/author") == 0)
	  ...
	  if (strcmp (current_string->array, "notes") == 0)
	    {
	      notes = 1;
	      cdata = 1;
	    }
	  ...
	  if (strcmp (current_string->array, "states") == 0)
	    {
	      states = 1;
	    }
	  if (strcmp (current_string->array, "/states") == 0)
	  ...
	  if (strcmp (current_string->array, "func") == 0)
	    {
	      func = 1;
	    }			/*charlist = NULL; */
	  if (strcmp (current_string->array, "/func") == 0)
	  ...
	  if (strcmp (current_string->array, "dest") == 0
	  ...
	  if (strcmp (current_string->array, "messages") == 0)
	    {
	      messages = 1;
	    }
	  if (strcmp (current_string->array, "/messages") == 0)
	  ...
	  while (current_function)
	      ...
	      while (current_adj_function)
		{
		  ...
		  if (strcmp (current_adj_function->type, "internal") != 0)
			...
			{
			  current_ioput =
			    addioput (&current_adj_function->function->
				      outputs);
			  current_ioput->messagetype =
			    copystr (current_adj_function->type);
			  current_ioput->function = current_function;
  ...
  while (current_xmachine)
      ...
      while (current_function)
	{
	  ...
	  while (current_ioput)
		...
		{
		  fprintf (stderr,
			   "ERROR: output message type '%s' in function '%s' in agent '%s' in file '%s' doesn't exist\n",
			   current_ioput->messagetype, current_function->name,
			   current_xmachine->name, current_function->file);
		  return -1;
		}
