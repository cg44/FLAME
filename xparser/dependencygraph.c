...
const char *colour_map[] =
  { "blue", "blueviolet", "brown", "burlywood4", "cadetblue", "chartreuse",
"darkgreen", "deeppink", "cyan4", "firebrick", "gold", "lightpink3", "lightslateblue", "magenta",
"red", "yellow" };
const char *colour_map_light[] =
  { "antiquewhite", "aquamarine", "azure3", "brown1", "chartreuse1",
"orchid1", "lightsalmon4", "darkgoldenrod1", "darkkhaki", "darkolivegreen3", "darkslateblue",
"forestgreen", "gray70", "indianred", "lightslateblue", "maroon3" };

void
latex_print_to_file (char *text, FILE * file)
{
  int i;
 ...
 */
void
output_latex (char *filename, char *filepath, model_data * modeldata)
{
  /* File to write to */
  FILE *file;
  ...
  for (current_xmachine = *modeldata->p_xmachines; current_xmachine != NULL;
	...
	("{{\\bfseries \\tablename\\ \\thetable{} -- continued from previous page}} \\\\\n",
	 file);
      ...
      for (current_variable = current_xmachine->variables;
  ...
  for (current_datatype = *modeldata->p_datatypes; current_datatype != NULL;
       current_datatype = current_datatype->next)
    {
      ...
      for (current_variable = current_datatype->vars;
	   current_variable != NULL;
	   current_variable = current_variable->next)
	{
	  fputs ("    \\url{", file);
 ...
 */
void
output_dgraph (char *filename, char *filepath, model_data * modeldata)
{
  /* File to write to */
  FILE *file;
  /* Buffer for concatenating strings */
 ...
 */
void
printRule (rule_data * current_rule_data, FILE * file)
{
  char buffer[1000];
  ...
  if (current_rule_data != NULL)
      ...
      else
	{
	  ...
	  if (current_rule_data->lhs != NULL)
	    ...
	    printRule (current_rule_data->lhs_rule, file);

	  if (current_rule_data->op != NULL)
	    {
	      fputs (" ", file);
	      fputs (current_rule_data->op_print, file);
	      if (strcmp (current_rule_data->op, "&&") == 0
		  || strcmp (current_rule_data->op, "||") == 0)
		fputs ("\\n", file);
	      else
		fputs (" ", file);
	    }
	  ...
	  if (current_rule_data->rhs != NULL)
	    fputs (current_rule_data->rhs_print, file);
	  else
	    printRule (current_rule_data->rhs_rule, file);
	}
 ...
 */
void
output_stategraph (char *filename, char *filepath, model_data * modeldata,
		   int flag)
{
  /* File to write to */
  FILE *file;
  ...
  while (current_message)
      ...
      while (current_sync)
	{
	  ...
	  sprintf (buffer, "%d",
		   current_sync->firstdependent->function->rank_in);
	  fputs (buffer, file);
	  fputs ("\" color=\"#00ff00\" shape = parallelogram];\n", file);
 ...
 */
void
output_communication_graph (char *filename, char *filepath,
			    model_data * modeldata)
{
  ...
  fputs
    ("/* circo -Tps communication_graph.dot -o communication_graph.ps */\n",
     file);
  ...
  while (current_xmachine)
	...
	{
	  if (current_xmachine == current_xmachine2)
	    flag = 1;
	  ...
	  if (flag == 1)
		...
		{
		  if (strcmp
		      (current_communication->output_function->agent_name,
		       current_xmachine->name) == 0
		      && strcmp (current_communication->input_function->
				 agent_name, current_xmachine2->name) == 0)
		    {
		      count++;
		    }
		  ...
		  if (strcmp
		      (current_communication->output_function->agent_name,
		       current_xmachine2->name) == 0
		      && strcmp (current_communication->input_function->
				 agent_name, current_xmachine->name) == 0)
		    {
		      count++;
		    }
...
void
output_process_order_graph (char *filename, char *filepath,
			    model_data * modeldata)
{
  /* File to write to */
  FILE *file;
  ...
  while (current_message)
      ...
      while (current_sync)
	{
	  ...
	  while (current_function_pointer)
	    {
	      ...
	      fputs (current_function->next_state, file);
	      fputs
		(" [ color=\"#00ff00\", constraint=false, style=dashed ];\n",
		 file);

	      current_function_pointer = current_function_pointer->next;
	    }
	  current_function_pointer =
	    current_sync->filter_variable_changing_functions;
	  while (current_function_pointer)
	    {
	      current_function = current_function_pointer->function;
	  ...
	  current_function_pointer =
	    current_sync->previous_sync_inputting_functions;
	  while (current_function_pointer)
	    {
	      current_function = current_function_pointer->function;
...
void
output_stategraph_colour (char *filename, char *filepath,
			  model_data * modeldata, int flag)
{
  /* File to write to */
  FILE *file;
  /* Buffer for concatenating strings */
  ...
  while (current_xmachine)
      ...
      while (current_function)
	{
	  ...
	  addmodel_colour (&p_model_colours, current_function->file,
			   length_colour_int);
	  fputs ("\t", file);
	  fputs (current_xmachine->name, file);
	  fputs ("_", file);
	  ...
	  model_display_colour =
	    displaymodel_colour (&p_model_colours, current_function->file);
...
int
find_loop (xmachine_function * current, xmachine_function * depends)
{
  adj_function *current_adj_function;
  ...
  /* If there is no self dependency (yet) then try next layer of dependencies */
      ...
      while (current_adj_function)
	{
	  ...
	  while (current_adj_function2)
	    {
	      if (current_adj_function2->function ==
...
void
find_previous_states (xmachine_function * current_function,
		      xmachine_message * current_message)
{
  adj_function *current_adj_function;
  ...
  while (current_adj_function)
    {
	  ...
	  (current_function->agent_name,
	   current_adj_function->function->agent_name) == 0)
	find_previous_states (current_adj_function->function,
			      current_message);
 ...
 */
void
calculate_filter_agent_states (model_data * modeldata)
{
 ...
 */
int
find_agent_start_states (model_data * modeldata)
{
  xmachine *current_xmachine;
  xmachine *current_xmachine2;
  xmachine_function *current_function;
  ...
  while (current_xmachine)
      ...
      while (current_state)
	{
	  ...
	  while (current_function)
	    {
	      if (strcmp (current_function->next_state, current_state->name)
		  == 0)
		k = 1;

	      if (strcmp
		  (current_function->current_state, current_state->name) == 0)
		m++;
	  ...
	  if (k == 0)
		...
		{
		  fprintf (stderr,
			   "ERROR: multiple start states found in '%s' agent\n",
			   current_xmachine->name);
		  fprintf (stderr, "\tincludes %s and %s states\n",
			   current_xmachine->start_state->name,
			   current_state->name);
		  return -1;
		}
	    }
      ...
      if (current_xmachine->start_state == NULL)
	{
	  /*fprintf(stderr, "ERROR: no start state found in '%s' agent\n", current_xmachine->name);
	     return -1; */
	  fprintf (stderr,
		   "!!! WARNING: no start state found in '%s' agent, agent removed from model\n",
		   current_xmachine->name);
 ...
 */
void
catalogue_agent_communications (model_data * modeldata)
{
  xmachine *current_xmachine;
  xmachine *current_xmachine2;
  xmachine_function *current_function;
		      ...
		      while (current_output)
			{
			  ...
			  if (strcmp
			      (current_input->messagetype,
			       current_output->messagetype) == 0)
			    {
			      /* Add the communication to the communication list */
			      add_flame_communication (current_input->
						       messagetype,
						       current_input->
						       filter_rule,
						       current_function,
			      ...
			      add_adj_function (current_function2,
						current_function,
						current_input->messagetype);
			    }
...
int
check_message_consistancy (model_data * modeldata)
{
  xmachine_message *current_message;
  ...
  while (current_message != NULL)
	  ...
	  for (current_function = current_xmachine->functions;
	       ...
	       current_function = current_function->next)
	    {
	      for (current_ioput = current_function->outputs;
		   current_ioput != NULL; current_ioput = current_ioput->next)
		{
		  if (strcmp
		      (current_message->name,
	...
	printf
	  ("!!! WARNING: %s message is not input by any agent function\n",
	   current_message->name);
      if (input_flag == 1 && output_flag == 0)
	printf
	  ("!!! WARNING: %s message is not output by any agent function\n",
	   current_message->name);
      if (input_flag == 0 && output_flag == 0)
	{
	  printf
	    ("!!! WARNING: %s message is not input or output by any agent function, message removed from model\n",
	     current_message->name);
 ...
 */
void
catalogue_agent_internal (model_data * modeldata)
{
  xmachine *current_xmachine;
  xmachine_function *current_function;
  xmachine_function *current_function2;
	  ...
	  while (current_function2)
	    {
		  ...
		  (current_function->current_state,
		   current_function2->next_state) == 0)
		{
		  ...
		  add_adj_function (current_function2, current_function,
				    "internal");
		}
 ...
 */
int
check_dependency_loops (model_data * modeldata)
{
  xmachine *current_xmachine;
  xmachine_function *current_function;
  adj_function *current_adj_function;
...
void
catalogue_states_edges (model_data * modeldata)
{
  xmachine_state *current_state;
  xmachine *current_xmachine;
  xmachine_function *current_function;
	  ...
	  while (current_function)
	    {
		  ...
		  (current_function->current_state, current_state->name) == 0)
		{
		  addfunction_pointer (&current_state->outgoing_functions,
				       current_function);
		}
	      if (strcmp (current_function->next_state, current_state->name)
		  == 0)
		{
		  addfunction_pointer (&current_state->incoming_functions,
				       current_function);
		}
...
void
find_states_with_branches (xmachine_state_holder ** states,
			   model_data * modeldata)
{
  ...
  while (current_xmachine)
      ...
      while (current_function)
	{
	  ...
	  while (current_function2)
	    {
	      if (strcmp (current_function->name, current_function2->name) !=
		  0
		  && strcmp (current_function->current_state,
			     current_function2->current_state) == 0)
		{
		  current_state = current_xmachine->states;
		  while (current_state)
		    {
		      if (strcmp
			  (current_state->name,
			   current_function->current_state) == 0)
			{
			  found = 0;
			  ...
			  while (current_state_holder)
			    {
			      if (current_state ==
				  current_state_holder->state)
				{
				  found = 1;
				}

			      current_state_holder =
				current_state_holder->next;
			    }
			  if (found == 0)
			    {
 ...
 */
void
calculate_dependency_graph (model_data * modeldata)
{
		  ...
		  if (k == 0)
		    {
		      ...
		      addfunction_pointer (&current_layer->functions,
					   current_function);
			...
			{
			  current_function_pointer =
			    current_state_holder->state->incoming_functions;
			  while (current_function_pointer)
			    {
			      if (current_function ==
				  current_function_pointer->function)
				{
				  //printf("** found %s\n", current_function_pointer->function->name);
      ...
      else
	{
	  ...
	  while (current_state_holder)
	      ...
	      if (current_state_holder->state->incoming_functions == NULL)
		{
		  ...
		  addstateholder (current_state_holder->state,
				  &current_layer->branching_states);
	  ...
	  while (current_state_holder)
		...
		current_state_holder->state->incoming_functions;
	      while (current_function_pointer)
		{
		  if (current_function_pointer->found == 0)
		    k = 1;
...
int
handle_rule_value_for_agent_variable (char *value, variable * var,
				      xmachine * current_xmachine, int flag)
{
  ...
  if (strncmp (value, "a->", 3) == 0)
      ...
      if (flag == 2)
	{
      ...
      for (current_variable2 = current_xmachine->variables;
	   current_variable2 != NULL;
	   current_variable2 = current_variable2->next)
	{
	  if (strcmp (var->name, current_variable2->name) == 0)
	    found_flag = 1;
	}
...
int
handle_rule_for_agent_variable (rule_data * current_rule_data,
				xmachine * current_xmachine, int flag)
{
  int rc;
  ...
  if (current_rule_data->lhs == NULL)
    rc =
      handle_rule_for_agent_variable (current_rule_data->lhs_rule,
				      current_xmachine, flag);
  else
    rc =
      handle_rule_value_for_agent_variable (current_rule_data->lhs,
					    current_rule_data->lhs_variable,
					    current_xmachine, flag);
  if ((flag == 0 || flag == 2) && rc == 1)
    return 1;

  if (current_rule_data->rhs == NULL)
    rc =
      handle_rule_for_agent_variable (current_rule_data->rhs_rule,
				      current_xmachine, flag);
  else
    rc =
      handle_rule_value_for_agent_variable (current_rule_data->rhs,
...
int
handle_rule_for_message_variable (rule_data * current_rule_data)
{
  if (current_rule_data->lhs == NULL)
    {
      if (handle_rule_for_message_variable (current_rule_data->lhs_rule) == 1)
	return 1;
    }
  else if (strncmp (current_rule_data->lhs, "m->", 3) == 0)
    return 1;

  if (current_rule_data->rhs == NULL)
    {
      if (handle_rule_for_message_variable (current_rule_data->rhs_rule) == 1)
	return 1;
    }
  else if (strncmp (current_rule_data->rhs, "m->", 3) == 0)
    return 1;
...
void
calculate_message_input_output_agents (model_data * modeldata)
{
  xmachine *current_xmachine;
  xmachine *current_xmachine2;
  ...
  for (current_message = *modeldata->p_xmessages;
       current_message != NULL; current_message = current_message->next)
    {
      for (current_xmachine = *modeldata->p_xmachines;
	   current_xmachine != NULL;
	   current_xmachine = current_xmachine->next)
		   ...
		   current_ioput != NULL; current_ioput = current_ioput->next)
		{
		  if (strcmp
		       ...
		       current_ioput->messagetype) == 0)
		    {
		      ...
		      flag = 0;
		      for (current_xmachine2 =
			   current_message->inputting_agents;
			   current_xmachine2 != NULL;
			   current_xmachine2 = current_xmachine2->next)
			{
			  if (strcmp
			      (current_xmachine2->name,
			       current_xmachine->name) == 0)
			    flag = 1;
			}
		      if (flag == 0)
			{
			  current_xmachine2 =
			    addxmachine (&current_message->inputting_agents,
					 current_xmachine->name);
			  current_xmachine2->start_state =
			    current_xmachine->start_state;
			}
		    }
		}
		   ...
		   current_ioput != NULL; current_ioput = current_ioput->next)
		{
		  if (strcmp
		       ...
		       current_ioput->messagetype) == 0)
		    {
		      ...
		      flag = 0;
		      for (current_xmachine2 =
			   current_message->outputting_agents;
			   current_xmachine2 != NULL;
			   current_xmachine2 = current_xmachine2->next)
			{
			  if (strcmp
			...
			{
			  current_xmachine2 =
			    addxmachine (&current_message->outputting_agents,
					 current_xmachine->name);
			  current_xmachine2->start_state =
			    current_xmachine->start_state;
			}
...
void
calculate_communication_syncs (model_data * modeldata)
{
  xmachine *current_xmachine;
  variable *current_variable;
	      ...
	      while (current_output)
		{
		  if (strcmp
		       ...
		       current_output->messagetype) == 0)
		    {
		      ...
		      addfunction_pointer (&temp_function_pointer,
					   current_function);
		    }
      ...
      if (temp_function_pointer != NULL)
	{
		  ...
		  while (current_input)
			   ...
			   current_input->messagetype) == 0)
			{
			  ...
			  if (current_message->syncs != NULL)
			    {
			      addfunction_pointer (&current_sync->
						   inputting_functions,
						   current_function);
			    }
			  /* else add a new sync */
			      ...
			      if (current_sync != current_message->syncs)
				{
			      ...
			      current_function_pointer2 =
				temp_function_pointer;
			      while (current_function_pointer2)
				{
				  addfunction_pointer (&current_sync->
						       outputting_functions,
						       current_function_pointer2->
						       function);
				  current_function_pointer2 =
				    current_function_pointer2->next;
				}
			    }
	  ...
	  if (flag == 0)
	      ...
	      while (current_sync)
		{
		  ...
		  current_function_pointer =
		    current_sync->inputting_functions;
		  while (current_function_pointer)
		    {
		      current_function = current_function_pointer->function;
			      ...
			      if (current_input->filter_rule != NULL)
				{
				  ...
				  if (current_input->filter_rule->box > 0)
				  ...
				  else
				    {
				      ...
				      if (strcmp
					  (current_input->filter_rule->op,
					   "==") == 0)
					{
					  flag = 0;
					  if (current_input->filter_rule->
					      lhs_variable != NULL)
					    {
					      if (current_input->filter_rule->
						  lhs_variable->agent != NULL
						  && current_input->
						  filter_rule->lhs_variable->
						  constant == 1)
						flag = 1;
					    }
					  if (current_input->filter_rule->
					    ...
					    {
					      if (current_input->filter_rule->
						  rhs_variable->agent != NULL
						  && current_input->
						  filter_rule->rhs_variable->
						  constant == 1)
						flag = 1;
					    }
					  if (flag == 1)
						...
						filter_agent_count++;
					      ...
					      handle_rule_for_agent_variable
						(current_input->filter_rule,
						 current_xmachine, 1);
					      /* If filter has message vars update sync flag */
					      if (handle_rule_for_message_variable (current_input->filter_rule))
						current_sync->
						  has_agent_and_message_vars =
						  1;
						...
						(&current_xmachine->
						 functions);
					      current_function2->name =
						copystr (current_input->
							 filter_function);
					      current_function2->agent_name =
						copystr (current_function->
							 agent_name);
					      (void)
						add_rule_data
						(&current_function2->
					      ...
					      if (handle_rule_for_agent_variable (current_input->filter_rule, current_xmachine, 2))
						{
						  ...
						  while (current_adj_function)
							   ...
							   agent_name) == 0)
							{
							  ...
							  addfunction_pointer
							    (&current_sync->
							     filter_variable_changing_functions,
							     current_adj_function->
							     function);
		  ...
		  while (current_xmachine)
		      ...
		      while (current_variable)
			{
			  ...
			  if (flag == 0)
			    {
			      current_variable2 =
				addvariable (&current_sync->vars);
			      current_variable2->name =
				copystr (current_variable->name);
			      current_variable2->type =
				copystr (current_variable->type);
			      current_variable2->constant =
				current_variable->constant;
			      current_variable2->agent =
				current_variable->agent;
...
int
calculate_sync_lengths (model_data * modeldata)
{
  /*
  ...
  while (current_message)
	...
	{
	  total +=
	    (current_sync->firstdependent->function->index -
	     current_sync->lastdepend->function->index);
...
void
order_functions_in_process_layers (model_data * modeldata)
{
      ...
      while (current_function_pointer)
	{
	  ...
	  while (current_message)
	      ...
	      while (current_sync)
		{
		  ...
		  current_function_pointer2 =
		    current_sync->outputting_functions;
		  while (current_function_pointer2)
		    {
	  ...
	  if (input_score > output_score)
	    current_function->score = input_score;
	  else if (input_score < output_score)
	    current_function->score = -output_score;
	  else
	  ...
	  if (current_layer->functions == NULL)
	      ...
	      while (current_function_pointer2)
		{
		  ...
		  if ((current_function->score > 0 &&
		       ...
		       current_function->score >
		       current_function_pointer2->function->score)
		      || (current_function->score < 0
			  && current_function_pointer2->function->score < 0
			  && current_function->score >
			  current_function_pointer2->function->score)
		...
		{
		  current_function_pointer->next =
		    previous_function_pointer->next;
...
void
assign_function_order_index_and_sync_dependency_functions (model_data *
							   modeldata)
{
  layer *current_layer;
  xmachine_function *current_function;
      ...
      while (current_sync)
	{
	  ...
	  while (current_function_pointer)
	      ...
	      if (last_dependent_function == NULL)
		{
	      ...
	      else if (current_function->index >
		       last_dependent_function->index)
		{
		  last_dependent_function = current_function;
		}
...
void
calculate_message_start_end_syncs (model_data * modeldata)
{
  /*  xmachine_message * current_message;
     xmachine_function * current_function;
     xmachine_function * current_function2;
...
void
apply_sync_data_to_functions (model_data * modeldata)
{
  xmachine_message *current_message;
  sync *current_sync;
  function_pointer *current_function_pointer;
	  ...
	  while (current_function_pointer)
	    {
	      ...
	      /*current_ioput = addioput(&current_function->first_inputs);
	         current_ioput->messagetype = copystr(current_message->name);
	         current_ioput->message = current_message; */
	      add_sync_pointer (&current_function->complete_syncs,
				current_sync);
...
void
calculate_partition_data (model_data * modeldata)
{
  xmachine_message *current_message;
  xmachine *current_xmachine;
  xmachine_function *current_function;
...
void
print_sync_data (model_data * modeldata)
{
  xmachine_message *current_message;
  sync *current_sync;
  xmachine *current_xmachine;
 ...
 */
void
find_constant_filter_vars (model_data * modeldata)
{
  xmachine_message *current_message;
  sync *current_sync;
  variable *current_variable;
		  ...
		  if (current_variable->constant == 1)
		    {
		      ...
		      while (current_variable2)
			{
			  if (current_variable->agent ==
			      current_variable2->agent
			      && strcmp (current_variable->name,
					 current_variable2->name) == 0)
			    flag = 1;
			...
			{
			  current_variable2 =
			    addvariable (modeldata->p_constant_filter_vars);
			  current_variable2->name =
			    copystr (current_variable->name);
			  current_variable2->type =
			    copystr (current_variable->type);
...
void
calculate_last_read_layer_of_message_types (model_data * modeldata)
{
  xmachine_message *current_message;
  ...
  for (current_message = *modeldata->p_xmessages;
       current_message != NULL; current_message = current_message->next)
    {
      ...
      for (current_layer = *modeldata->p_layers;
	   current_layer != NULL; current_layer = current_layer->next)
	{
	  /* For each function */
	  for (current_function_pointer = current_layer->functions;
	       ...
	       current_function_pointer = current_function_pointer->next)
	    {
	      ...
	      for (current_input = current_function->inputs;
		   current_input != NULL; current_input = current_input->next)
		{
		  if (strcmp
		      (current_message->name,
		       current_input->messagetype) == 0)
 ...
 */
int
create_dependency_graph (char *filepath, model_data * modeldata)
{
  int rc;
