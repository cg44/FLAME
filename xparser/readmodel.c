 ...
 */
int
checkRuleAgentVar (rule_data * current_rule_data)
{
  int flag = 0;

  if (current_rule_data->time_rule == 1)
    {
      if (strncmp (current_rule_data->rhs, "a.", 2) == 0)
	flag = 1;
    }
  else
    {
	...
	{
	  if (checkRuleAgentVar (current_rule_data->lhs_rule) == 1)
	    flag = 1;
	}
      else
	{
	  if (strncmp (current_rule_data->lhs, "a.", 2) == 0)
	    flag = 1;
	}

      if (current_rule_data->rhs == NULL)
	{
	  if (checkRuleAgentVar (current_rule_data->rhs_rule) == 1)
	    flag = 1;
	}
      else
	{
...
int
checkRuleMessageVar (rule_data * current_rule_data)
{
  int flag = 0;

  if (current_rule_data->lhs == NULL)
    {
      if (checkRuleMessageVar (current_rule_data->lhs_rule) == 1)
	flag = 1;
    }
  else
    {
      if (strncmp (current_rule_data->lhs, "m.", 2) == 0)
	flag = 1;
    }
  ...
  if (current_rule_data->rhs == NULL)
    ...
    {
      if (strncmp (current_rule_data->rhs, "m.", 2) == 0)
	flag = 1;
    }
...
void
handleVariableType (char_array * current_string, variable * current_variable,
		    model_data * modeldata)
{
  ...
  if (strcmp (current_variable->type, "float") == 0)
    strcpy (current_variable->mpi_type, "MPI_FLOAT");
  if (strcmp (current_variable->type, "char") == 0)
    strcpy (current_variable->mpi_type, "MPI_CHAR");
  if (strcmp (current_variable->type, "short int") == 0)
    strcpy (current_variable->mpi_type, "MPI_SHORT");
  if (strcmp (current_variable->type, "long int") == 0)
    strcpy (current_variable->mpi_type, "MPI_LONG");
  if (strcmp (current_variable->type, "long double") == 0)
    strcpy (current_variable->mpi_type, "MPI_LONG_DOUBLE");
  if (strcmp (current_variable->type, "unsigned int") == 0)
    strcpy (current_variable->mpi_type, "MPI_UNSIGNED");
  if (strcmp (current_variable->type, "unsigned short int") == 0)
    strcpy (current_variable->mpi_type, "MPI_UNSIGNED_SHORT");
  if (strcmp (current_variable->type, "unsigned long int") == 0)
    strcpy (current_variable->mpi_type, "MPI_UNSIGNED_LONG");
  if (strcmp (current_variable->type, "unsigned char") == 0)
    strcpy (current_variable->mpi_type, "MPI_UNSIGNED_CHAR");
...
void
handleVariableName (char_array * current_string, variable * current_variable)
{
  int j, i = 0;
  char buffer[100];
 ...
 */
void
readModel (input_file * inputfile, char *directory, model_data * modeldata)
{
  ...
  int header, iteration_end_code, depends, datatype, desc, cur_state,
    next_state;
  int input, output, messagetype, timetag, unit, period, lhs, op, rhs,
    condition;
  ...
  while (reading == 1 && c != (char) EOF)
	  ...
	  if (xmlcode != 0)
	    {
	      if ((xmlcode == 1) && (c == ']'))
		xmlcode = 2;
	      else if ((xmlcode == 11) && (c == '!'))
		xmlcode = 2;
	      else if ((xmlcode == 11) && (c == 'f'))
		xmlcode = 22;
	      else if (xmlcode == 2 && (c == '>' || c == '['))
		{
	      ...
	      else if (xmlcode == 3 && c == 'C')
		xmlcode = 4;
	      else if (xmlcode == 4 && c == 'D')
		xmlcode = 5;
	      else if (xmlcode == 5 && c == 'A')
		xmlcode = 6;
	      else if (xmlcode == 6 && c == 'T')
		xmlcode = 7;
	      else if (xmlcode == 7 && c == 'A')
		xmlcode = 8;
	      else if (xmlcode == 8 && c == '[')
		xmlcode = 9;
	      else if (xmlcode == 22 && c == 'i')
		xmlcode = 23;
	      else if (xmlcode == 23 && c == 'l')
		xmlcode = 24;
	      else if (xmlcode == 24 && c == 'e')
		xmlcode = 25;
      ...
      else if (c == '>')
	{
	  ...
	  if (current_string->array[current_string->size - 1] != '/')
	      ...
	      else
		{
		  ...
		  if (strcmp (&current_string->array[1], &chartag[numtag][0])
		      != 0 && strcmp (current_string->array, "/xmodel") != 0)
		    {
		      fprintf (stderr,
			       "ERROR: The tag <%s> on line number %i\n",
			       current_string->array, linenumber);
		      fprintf (stderr,
			       "ERROR: doesn't close the tag <%s> on line number %i\n",
	  ...
	  if (strcmp (current_string->array, "xmachine_agent_model") == 0
	      || strcmp (current_string->array, "xmodel") == 0)
	    {
	    }
	  if (strcmp (current_string->array, "/xmachine_agent_model") == 0
	      || strcmp (current_string->array, "/xmodel") == 0)
	  ...
	  if (strcmp (current_string->array, "var") == 0
	      || strcmp (current_string->array, "variable") == 0)
	    {
	      var = 1;
	      if (datatype == 1)
		current_variable = addvariable (p_variable);
	      else if (environment == 1)
		current_variable = addvariable (modeldata->p_envvars);
	      else if (memory == 1)
		{
		  current_variable =
		    addvariable (&current_xmachine->variables);
	  ...
	  if (strcmp (current_string->array, "datatype") == 0
	      || strcmp (current_string->array, "dataType") == 0)
	    {
	      datatype = 1;
	  ...
	  if (strcmp (current_string->array, "/datatype") == 0
	      ...
	      while (current_variable)
		{
		  if ((strcmp (current_variable->type, "int") == 0 ||
		      ...
		      current_variable->arraylength == 0)
		    current_datatype->has_single_vars = 1;
		  ...
		  if (current_variable->arraylength == -1)
		    current_datatype->has_dynamic_arrays = 1;
		  if (current_variable->ismodeldatatype == 1
		      && current_variable->datatype->has_dynamic_arrays == 1)
		    current_datatype->has_dynamic_arrays = 1;
	  ...
	  if (strcmp (current_string->array, "current_state") == 0
	      || strcmp (current_string->array, "currentState") == 0)
	    {
	      cur_state = 1;
	    }
	  if (strcmp (current_string->array, "/current_state") == 0
	  ...
	  if (strcmp (current_string->array, "next_state") == 0
	      || strcmp (current_string->array, "nextState") == 0)
	    {
	      next_state = 1;
	    }
	  if (strcmp (current_string->array, "/next_state") == 0
	  ...
	  if (strcmp (current_string->array, "messagetype") == 0
	      || strcmp (current_string->array, "messageName") == 0)
	    {
	      messagetype = 1;
	    }
	  if (strcmp (current_string->array, "/messagetype") == 0
	  ...
	  if (strcmp (current_string->array, "function") == 0)
	      ...
	      else
		{
		  ...
		  current_function =
		    addxfunction (&current_xmachine->functions);
		  fcode = NULL;
	  ...
	  if (strcmp (current_string->array, "/function") == 0)
		...
		addxstate (current_function->current_state,
			   current_function->agent_name,
			   &current_xmachine->states);
	      if (current_function->next_state != NULL)
		addxstate (current_function->next_state,
			   current_function->agent_name,
			   &current_xmachine->states);
	  ...
	  if (strcmp (current_string->array, "desc") == 0
	      || strcmp (current_string->array, "description") == 0)
	    {
	      desc = 1;
	    }
	  if (strcmp (current_string->array, "/desc") == 0
	  ...
	  if (strcmp (current_string->array, "/message") == 0)
	      ...
	      while (current_variable)
		{
		  ...
		  if (current_variable->arraylength == -1
		      || (current_variable->ismodeldatatype == 1
			  && current_variable->datatype->has_dynamic_arrays ==
			  1))
		    {
		      fprintf (stderr,
			       "Error: %s - dyamic array found in message\n",
		...
		{
		  fprintf (stderr,
			   "Error: Dynamic array found in %s message\n",
	  ...
	  if (strcmp (current_string->array, "timeunit") == 0
	      || strcmp (current_string->array, "timeUnit") == 0)
	    {
	      timetag = 1;
	    }
	  if (strcmp (current_string->array, "/timeunit") == 0
	    ...
	    {
	      timetag = 0;
	      add_time_unit (time_name, unit_name, period_int,
			     modeldata->p_time_units);
	    }
	  ...
	  if (strcmp (current_string->array, "time") == 0)
	      ...
	      if (lhs || rhs)
		{
		  ...
		  if (lhs_last)
		    {
		      current_rule_data =
			add_rule_data (&current_rule_data->lhs_rule);
		    }
		  else
		    {
		      current_rule_data =
			add_rule_data (&current_rule_data->rhs_rule);
		    }
		}
	      else
		{		/*if(condition) */
		  current_rule_data =
		    add_rule_data (&current_function->condition_rule);
		}
	  ...
	  if (strcmp (current_string->array, "lhs") == 0)
	      ...
	      if (current_rule_data == NULL)
		{
		  ...
		  if (filter)
		    {
		      current_rule_data =
			add_rule_data (&current_ioput->filter_rule);
		    }
		  /*last_rule_data = NULL; */
		}
	      else
	  ...
	  if (strcmp (current_string->array, "depends") == 0)
	    {
	      depends = 1;
	      /*charlist = NULL; */
	      current_adj_function =
		add_depends_adj_function (current_function);
	  ...
	  if (strcmp (current_string->array, "/filter") == 0)
	    {
	      ...
	      current_ioput->filter_rule->has_agent_var =
		checkRuleAgentVar (current_ioput->filter_rule);
	      current_ioput->filter_rule->has_message_var =
		checkRuleMessageVar (current_ioput->filter_rule);
	  ...
	  if (strcmp (current_string->array, "box2d") == 0)
		...
		{
		  fprintf (stderr,
			   "ERROR: The tag box2d must be used after a filter tag only\n");
		  exit (1);
		}
	      box2d = 1;
	  ...
	  if (strcmp (current_string->array, "box3d") == 0)
		...
		{
		  fprintf (stderr,
			   "ERROR: The tag box3d must be used after a filter tag only\n");
		  exit (1);
		}
	      box3d = 1;
	      current_rule_data = add_rule_data (&current_ioput->filter_rule);
      ...
      else if (c == '<')
	{
	  ...
	  if (model)
	      ...
	      if (enabled)
		{
		  ...
		  if (strcmp (temp_char, "true") == 0)
		    current_input_file->enabled = 1;
		  if (strcmp (temp_char, "false") == 0)
		    current_input_file->enabled = 0;
		  free (temp_char);
		}
	      if (codefile)
		{
		  ...
		  while (current_input_file->fulldirectory[i] != '\0')
		    {
		      ...
		      if (current_input_file->fulldirectory[i] == '\\')
			lastd = i;
		  ...
		  if (lastd != 0)
		    ...
		    current_input_file->fulldirectory[0] = '\0';
		  ...
		  while (current_input_file->localdirectory[i] != '\0')
		    {
		      /* For windows directories */
		      if (current_input_file->localdirectory[i] == '\\')
			lastd = i;
		      /* For unix directories */
		      if (current_input_file->localdirectory[i] == '/')
			lastd = i;
		  ...
		  if (lastd != 0)
		    ...
		    current_input_file->localdirectory[0] = '\0';
		}
	    }
	  if (environment)
	    {
		  ...
		  if (var)
		    {
		      if (type)
			{
			  handleVariableType (current_string,
					      current_variable, modeldata);
			}
		      if (name)
			{
			  handleVariableName (current_string,
					      current_variable);
			}
		      if (desc)
			current_variable->description =
			  copy_array_to_str (current_string);
		    }
		  else if (name)
		    {
		  ...
		  if (name)
		    {
		      current_variable->name =
			copy_array_to_str (current_string);
		      if ((strcmp (current_variable->name, "GSL_RNG_SEED") ==
			   0)
			  || (strcmp (current_variable->name, "gsl_rng_seed")
			      == 0))
			{
			  printf
			    ("- Environment variable (%s) found: using GSL\n",current_variable->name);
			  modeldata->gsl_lib = 1;
			}
		    }
		  if (desc)
		...
		{
		  if (name)
		    current_envdefine->name =
		      copy_array_to_str (current_string);
		  if (value)
		    {
		      strcpy (current_envdefine->value, "");
		      current_envdefine->value =
			copy_array_to_str (current_string);
		    }
	      ...
	      if (code && function)
		{
	      ...
	      if (header)
		current_envfunc->header = 1;
	    }
	  else if (var)
	    {
	      if (type)
		{
		  handleVariableType (current_string, current_variable,
				      modeldata);
		}
	      if (name)
		{
	      ...
	      if (constant)
		{
		  temp_char = copy_array_to_str (current_string);
		  if (strcmp (temp_char, "true") == 0)
		    current_variable->constant = 1;
		  if (strcmp (temp_char, "false") == 0)
		    current_variable->constant = 0;
		  free (temp_char);
		}
	      if (desc)
		current_variable->description =
	   ...
	   */
	      ...
	      if (codefile)
		{
	      ...
	      if (name && depends == 0)
		current_function->name = copy_array_to_str (current_string);
	      if (note)
		current_function->note = copy_array_to_str (current_string);
	      if (code)
		{
		  current_fcode->code = copy_array_to_str (current_string);
		}
	      if (cur_state)
		current_function->current_state =
		  copy_array_to_str (current_string);
	      if (next_state)
		current_function->next_state =
		  copy_array_to_str (current_string);
	      if (input)
		{
		  if (messagetype)
		  ...
		  if (sort)
		    {
		      if (key)
			current_ioput->sort_key =
			  copy_array_to_str (current_string);
		      if (order)
			current_ioput->sort_order =
			  copy_array_to_str (current_string);
		  ...
		  if (filter)
		      ...
		      else if (op)
			{
			  current_rule_data->op =
			    copy_array_to_str (current_string);
			  current_rule_data->op_print =
			    copy_array_to_str (current_string);
			}
		      else if (rhs && value)
			{
			  current_rule_data->rhs =
			    copy_array_to_str (current_string);
			  current_rule_data->rhs_print =
			    copy_array_to_str (current_string);
			}
		      else if (box2d || box3d)
			{
		...
		{
		  current_xmachine =
		    addxmachine (modeldata->p_xmachines,
      ...
      else if (intag)
	{
      ...
      /* If in data read char into buffer */
      else if (model || enabled || codefile
	       || ((iteration_end_code && code) || codefile || name || type
		   || desc || (memory
			       && (var
				   && (type || name || constant || desc))))
	       ...
	       || (function
		   && (not || name || note || code || depends || type
		       || cur_state || next_state || input || output
		       || messagetype || value || period || phase || var
		       || order || box2d || box3d)) || (define && (name
								   || value))
	       || (timetag && (name || unit || period)) || condition)
	{
	  /*current_charlist = addchar(p_charlist); */
	  /*current_charlist->character = c; */
	  add_char (current_string, c);
...
void
handleRuleValue (char **p_value, variable ** p_variable,
		 rule_data * current_rule_data,
		 xmachine_function * current_function,
		 xmachine * current_xmachine, char *messagetype,
		 model_data * modeldata)
{
  variable *current_variable;
  xmachine_message *current_message;
  ...
  if (strcmp (*p_value, "") == 0)
    {
      ...
      fprintf (stderr, "       in function '%s' %s->%s in agent '%s'\n",
	       current_function->name, current_function->current_state,
	       current_function->next_state, current_xmachine->name);
  ...
  if (strncmp (*p_value, "a.", 2) == 0)
	...
	{
	  fprintf (stderr,
		   "ERROR: value '%s' in filter rule not in agent memory\n",
		   buffer);
	  fprintf (stderr, "       in function '%s' %s->%s in agent '%s'\n",
		   current_function->name, current_function->current_state,
		   current_function->next_state, current_xmachine->name);
  ...
  /* If starts with 'm.' change to 'm->' and check rest is a valid message variable */
	...
	{
	  fprintf (stderr,
		   "ERROR: value '%s' in filter rule not in message variables\n",
		   buffer);
	  fprintf (stderr, "       in function '%s' %s->%s in agent '%s'\n",
		   current_function->name, current_function->current_state,
		   current_function->next_state, current_xmachine->name);
  ...
  /* If algorithm (i.e. radius check) then parser a. and m. to a-> and m-> */
      ...
      for (i = 0; i < (int) strlen (*p_value); i++)
	{
	  ...
	  if (i > 0)
	    {
	      if (((*p_value)[i - 1] == 'a' || (*p_value)[i - 1] == 'm')
		  && (*p_value)[i] == '.')
		{
		  buffer[j] = '-';
		  buffer[++j] = '>';
...
void
handleRule (rule_data * current_rule_data,
	    xmachine_function * current_function, xmachine * current_xmachine,
	    char *messagetype, model_data * modeldata)
{
  ...
  if (current_rule_data != NULL)
      ...
      if (current_rule_data->time_rule == 1)
	{
	  ...
	  while (current_time_unit)
	    {
	      if (strcmp (current_rule_data->lhs, current_time_unit->name) ==
		  0)
		{
	  ...
	  if (current_rule_data->op == NULL)
	    {
	      fprintf (stderr,
		       "ERROR: condition period '%s' is not a time unit\n",
		       current_rule_data->lhs);
	      exit (0);
	    }

	  handleRuleValue (&current_rule_data->rhs,
			   &current_rule_data->rhs_variable,
			   current_rule_data, current_function,
			   current_xmachine, messagetype, modeldata);
	}
      else if (current_rule_data->box > 0)
	{
	  if (current_rule_data->lhs != NULL)
	    handleRuleValue (&current_rule_data->lhs,
			     &current_rule_data->lhs_variable,
			     current_rule_data, current_function,
			     current_xmachine, messagetype, modeldata);
	  else
	    {
	      fprintf (stderr,
		       "ERROR: The value of a box filter is missing\n");
	      exit (0);
	    }
	}
      else
	{
	  ...
	  if (current_rule_data->lhs != NULL)
	    handleRuleValue (&current_rule_data->lhs,
			     ...
			     current_rule_data, current_function,
			     current_xmachine, messagetype, modeldata);
	  else if (current_rule_data->lhs_rule != NULL)
	    handleRule (current_rule_data->lhs_rule, current_function,
			current_xmachine, messagetype, modeldata);
	  else
	    {
	  ...
	  if (current_rule_data->rhs != NULL)
	    handleRuleValue (&current_rule_data->rhs,
			     ...
			     current_rule_data, current_function,
			     current_xmachine, messagetype, modeldata);
	  else if (current_rule_data->rhs_rule != NULL)
	    handleRule (current_rule_data->rhs_rule, current_function,
			current_xmachine, messagetype, modeldata);
	  else
	    {
	  ...
	  if (strcmp (current_rule_data->op, "EQ") == 0)
	    ...
	    {
	      fprintf (stderr,
		       "ERROR: '%s' operator not one of EQ/NEQ/LEQ/GEQ/LT/GT/AND/OR/IN\n",
		       current_rule_data->op);
	      fprintf (stderr,
		       "       in function '%s' %s->%s in agent '%s'\n",
		       current_function->name,
		       current_function->current_state,
		       current_function->next_state, current_xmachine->name);
	      fprintf (stderr, "       in file: '%s'\n",
	  ...
	  if (strcmp (current_rule_data->op, "IN") == 0)
	    {
	      ...
	      if (current_rule_data->lhs_variable->arraylength != 0
		  || strcmp (current_rule_data->lhs_variable->type,
			     "int") != 0)
		{
		  fprintf (stderr,
			   "ERROR: lhs value '%s' in filter rule using op 'IN' is not a single integer\n",
		...
		{
		  fprintf (stderr,
			   "ERROR: rhs value '%s' in filter rule using op 'IN' is not an array of integers\n",
			   current_rule_data->lhs_variable->name);
		  fprintf (stderr,
			   "       in function '%s' %s->%s in agent '%s'\n",
			   current_function->name,
...
int
checkmodel (model_data * modeldata)
{
  xmachine *current_xmachine;
  xmachine *current_xmachine2;
  variable *current_variable;
	...
	if (current_message != current_message2)
	  {
	    if (strcmp (current_message->name, current_message2->name) == 0)
	      {
		fprintf (stderr,
			 "ERROR: multiple definitions of message '%s'\n",
			 current_message->name);
		fprintf (stderr, "       in file: '%s'\n",
  ...
  while (current_xmachine)
      ...
      if (current_variable == NULL)
	{
	  /*fprintf(stderr, "ERROR: agent '%s' has no memory variables\n", current_xmachine->name);
	     return -1; */
	  fprintf (stderr,
		   "WARNING: no memory variables found in '%s' agent, agent removed from model\n",
		   current_xmachine->name);
	  /* Remove agent from the agent list */
	  if (current_xmachine2 == NULL)
	    *modeldata->p_xmachines = current_xmachine->next;
	  else
	    current_xmachine2->next = current_xmachine->next;
	  ...
	  if (current_xmachine2 == NULL)
	    ...
	    current_xmachine = current_xmachine2->next;
	}
      else
	{
	  ...
	  while (current_variable)
		  ...
		  || strcmp (current_variable->type, "char_array") == 0)
		{
	      ...
	      while (current_datatype)
		{
		  if (strcmp (current_variable->type, current_datatype->name)
		      == 0)
		    {
		      found = 1;
		    }
		...
		{
		  fprintf (stderr,
			   "ERROR: type variable '%s' in variable '%s' in agent '%s' doesn't exist\n",
			   current_variable->type, current_variable->name,
			   current_xmachine->name);
		  fprintf (stderr, "       in file: '%s'\n",
	  ...
	  while (current_variable)
		...
		{
		  if (strcmp (current_variable->name, current_variable2->name)
		      == 0 && current_variable != current_variable2)
		    {
		      fprintf (stderr,
			       "ERROR: multiple uses of variable '%s' in agent '%s'\n",
			       current_variable->name,
			       current_xmachine->name);
		      fprintf (stderr, "       in file: '%s'\n",
			       current_variable->file);
	  ...
	  while (current_variable)
	    {
	      if (current_variable->arraylength != 0)
		modeldata->agents_include_array_variables = 1;

	      /*copycharlist(&current_variable->name, &chardata[0]); */
	      sprintf (buffer, "%s_x", current_xmachine->name);
	      if (strcmp (current_variable->name, buffer) == 0)
		strcpy (current_xmachine->xvar, buffer);
	      if (strcmp (current_variable->name, "x") == 0)
		strcpy (current_xmachine->xvar, "x");
	      if (strcmp (current_variable->name, "px") == 0)
		strcpy (current_xmachine->xvar, "px");
	      if (strcmp (current_variable->name, "posx") == 0)
		strcpy (current_xmachine->xvar, "posx");
	      sprintf (buffer, "%s_y", current_xmachine->name);
	      if (strcmp (current_variable->name, buffer) == 0)
		strcpy (current_xmachine->yvar, buffer);
	      if (strcmp (current_variable->name, "y") == 0)
		strcpy (current_xmachine->yvar, "y");
	      if (strcmp (current_variable->name, "py") == 0)
		strcpy (current_xmachine->yvar, "py");
	      if (strcmp (current_variable->name, "posy") == 0)
		strcpy (current_xmachine->yvar, "posy");
	      sprintf (buffer, "%s_z", current_xmachine->name);
	      if (strcmp (current_variable->name, buffer) == 0)
		strcpy (current_xmachine->zvar, buffer);
	      if (strcmp (current_variable->name, "z") == 0)
		strcpy (current_xmachine->zvar, "z");
	      if (strcmp (current_variable->name, "pz") == 0)
		strcpy (current_xmachine->zvar, "pz");
	      if (strcmp (current_variable->name, "posz") == 0)
		strcpy (current_xmachine->zvar, "posz");

	      if (strcmp (current_variable->name, "range") == 0)
		strcpy (current_xmachine->rangevar, "range");
	      if (strcmp (current_variable->name, "radius") == 0)
		strcpy (current_xmachine->rangevar, "radius");

	      if (strcmp (current_variable->name, "id") == 0)
		strcpy (current_xmachine->idvar, "id");
	      if (strcmp (current_variable->name, "agent_id") == 0)
		strcpy (current_xmachine->idvar, "agent_id");
	      ...
	      while (allvar)
		{
		  ...
		  if (strcmp (current_variable->name, allvar->name) == 0)
			...
			{
			  fprintf (stderr,
				   "ERROR: variable '%s' defined twice but with different types\n",
				   current_variable->name);
			  return -1;
			}
		    }
	      ...
	      if (found == 0)
		{
		  ...
		  allvar->typenotarray =
		    copystr (current_variable->typenotarray);
		  strcpy (allvar->defaultvalue,
			  current_variable->defaultvalue);
  ...
  if (modeldata->depends_style == 1)
		  ...
		  if (current_function->rank_in == -1)
		      ...
		      while (current_adj_function)
			{
			  ...
			  if (current_adj_function->function->rank_in == -1
			      || current_adj_function->function->rank_in ==
			      newlayer)
			    {
		      ...
		      if (k == 0)
			{
			  ...
			  printf ("%s - %s - %d\n", current_xmachine->name,
				  current_function->name, newlayer);
      ...
      while (current_xmachine)
	{
	  ...
	  addxstate (current_function2->next_state,
		     current_function2->agent_name,
		     &current_xmachine->states);
	  ...
	  while (current_function)
	      ...
	      while (current_adj_function)
		{
		  ...
		  if (current_adj_function->function == NULL)
		    {
		      fprintf (stderr,
			       "ERROR: depends function '%s' does not exist\n",
			       current_adj_function->name);
		      return -1;
		    }
		  ...
		  if (strcmp (current_adj_function->type, "internal") != 0)
			...
			{
			  if (strcmp
			      (current_adj_function->type,
			       current_ioput->messagetype) == 0)
			    found = 1;
			...
			{
			  current_ioput =
			    addioput (&current_function->inputs);
			  current_ioput->function = current_function;
			  current_ioput->messagetype =
			    copystr (current_adj_function->type);
			}
  ...
  while (current_xmachine)
      ...
      while (current_function)
	{
	  ...
	  if (current_function->condition_rule != NULL)
	    {
	      ...
	      handleRule (current_function->condition_rule, current_function,
			  current_xmachine, NULL, modeldata);
	    }
	  ...
	  while (current_ioput)
		...
		{
		  fprintf (stderr,
			   "ERROR: input message type '%s' in function '%s' in agent '%s' in file '%s' doesn't exist\n",
			   current_ioput->messagetype, current_function->name,
			   current_xmachine->name, current_function->file);
		  return -1;
		}
	      ...
	      if (current_ioput->filter_rule != NULL)
		{
		  ...
		  handleRule (current_ioput->filter_rule, current_function,
			      current_xmachine, current_ioput->messagetype,
			      modeldata);
		  ...
		  if (current_ioput->filter_rule->box == 2)
			...
			{
			  fprintf (stderr,
				   "ERROR: agent '%s' contains a box2d filter but does not contain memory variables called x and y\n",
				   current_xmachine->name);
			  return -1;
			}
		    }
		  if (current_ioput->filter_rule->box == 3)
			...
			{
			  fprintf (stderr,
				   "ERROR: agent '%s' contains a box3d filter but does not contain memory variables called x, y and z\n",
				   current_xmachine->name);
			  return -1;
			}
		    }
		  ...
		  if (current_ioput->filter_rule->box > 0)
		    {
		      ...
		      if (strncmp (current_ioput->filter_rule->lhs, "a", 1) ==
			  0)
			{
			  current_ioput->box_apothem =
			    (char *)
			    malloc ((strlen (current_ioput->filter_rule->lhs)
				     + strlen (current_xmachine->name) +
				     strlen ("current_xmachine_") +
				     1) * sizeof (char));
			  strcpy (current_ioput->box_apothem,
				  "current_xmachine_");
			  strcat (current_ioput->box_apothem,
				  current_xmachine->name);
			  strcat (current_ioput->box_apothem,
				  current_ioput->filter_rule->lhs + 1);
			}
		      else
			{
			  current_ioput->box_apothem =
		  ...
		  || current_ioput->sort_order != NULL)
		{
		  if (current_ioput->sort_key == NULL)
		    {
		      fprintf (stderr,
			       "ERROR: sort of message type '%s' in function '%s' in agent '%s' in file '%s' doesn't have a sort variable\n",
			       current_ioput->messagetype,
			       current_function->name, current_xmachine->name,
			       current_function->file);
		      return -1;
		    }
		  if (current_ioput->sort_order == NULL)
		    {
		      fprintf (stderr,
			       "ERROR: sort of message type '%s' in function '%s' in agent '%s' in file '%s' doesn't have a sort order\n",
			       current_ioput->messagetype,
			       current_function->name, current_xmachine->name,
			       current_function->file);
		      return -1;
		  ...
		  for (current_variable = current_ioput->message->vars;
		       ...
		       current_variable = current_variable->next)
		    {
		      if (strcmp
			  (current_variable->name,
			   current_ioput->sort_key) == 0)
			found = 1;
		    }
		  if (found == 0)
		    {
		      fprintf (stderr,
			       "ERROR: sort of message type '%s' in function '%s' in agent '%s' in file '%s' doesn't have a valid sort variable '%s'\n",
			       current_ioput->messagetype,
			       current_function->name, current_xmachine->name,
			       current_function->file,
			       current_ioput->sort_key);
		  ...
		  if (strcmp (current_ioput->sort_order, "ascend") != 0 &&
		    ...
		    {
		      fprintf (stderr,
			       "ERROR: sort of message type '%s' in function '%s' in agent '%s' in file '%s' has an order '%s' that is not ascend or descend\n",
			       current_ioput->messagetype,
			       current_function->name, current_xmachine->name,
			       current_function->file,
			       current_ioput->sort_order);
  ...
  while (current_message)
      ...
      if (current_message->has_box2d)
	{
	  ...
	  if (has_x == 0 || has_y == 0)
	    {
	      fprintf (stderr,
		       "ERROR: message '%s' is involved in a box2d filter but does not contain variables called x and y\n",
		       current_message->name);
	      return -1;
	    }
      ...
      if (current_message->has_box3d)
	{
	  ...
	  if (has_x == 0 || has_y == 0 || has_z == 0)
	    {
	      fprintf (stderr,
		       "ERROR: message '%s' is involved in a box3d filter but does not contain variables called x, y and z\n",
		       current_message->name);
	      return -1;
	    }
	  current_message->make_x_function = 1;
