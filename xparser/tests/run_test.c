...
/*	printf("\n****Calling test_model_code_standard for %d\n",n);	*/

	sprintf(buffer, "splint -I/home/cg/SANDBOX/xagents/libmboard/include -I/usr/lib/openmpi/include -D_DEBUG_MODE -weak -namechecks -bufferoverflowhigh test%d/*.c", n);
		...
		else
		{
			while(fgets(buffer, 1000, out) != NULL)
			{
				if(strstr(buffer, "error: mboard.h: No such file or directory") != NULL)
				{
					...
					sprintf(buffer, "make --directory=test%d/ LIBMBOARD_DIR=/home/cg/SANDBOX/xagents/libmboard/", index);
